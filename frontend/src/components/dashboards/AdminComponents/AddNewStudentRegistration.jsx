...
const AddNewStudentRegistration = () => {
    const navigate = useNavigate();
    const { isSending, AddNewStudent, addNewRegistration } = useAdminStore();
    ...
    const validateForm = () => {
        ...
        if (!form.nationality.trim()) return toast.error("Nationality is required");
        if (!form.category.trim()) return toast.error("Category is required");
        if (!form.subCategory.trim()) return toast.error("sub Category is required");
        if (!form.quota.trim()) return toast.error("quota is required");
        if (!form.religion.trim()) return toast.error("religion is required");
        if (!form.email.trim()) return toast.error("email is required");
        if (!form.phone.trim()) return toast.error("phone is required");

        if (!form.fatherFirstName.trim()) return toast.error("father First Name is required");
        if (!form.fatherPhone.trim()) return toast.error("father Phone is required");

        if (!form.motherFirstName.trim()) return toast.error("mother FirstName is required");
        if (!form.motherPhone.trim()) return toast.error("mother Phone is required");

        if (!form.guardianFirstName.trim()) return toast.error("guardian FirstName is required");
    ...
    const handleFilePreviousSchoolLastClassMarksheet = (key) => (e) => {
        const file = e.target.files?.[0];
        if (file) {
            setForm({ ...form, [key]: file });
            setFilePreviews({ ...filePreviews, [key]: URL.createObjectURL(file) });
    ...
    const handleFilePreviousSchoolTC = (key) => (e) => {
        const file = e.target.files?.[0];
        if (file) {
            setForm({ ...form, [key]: file });
            setFilePreviews({ ...filePreviews, [key]: URL.createObjectURL(file) });
    ...
    //             </h2>
    //             <form onSubmit={handleSubmit} className="bg-white w-full p-6 sm:p-8 md:p-10 rounded-2xl shadow-lg ">
    //                 {/* Personal Details */}
...
export default AddNewStudentRegistration;
