...
const CropMotherImage = ({ student }) => {
    ...
    const { setMotherPic } = useAdminStore()
    ...
    const finalSubmit = async (e) => {
        ...
        try {
            ...
            const success = await setMotherPic(formData)
            if (success == true) {
        ...
        <div className=' bg-white w-full h-full rounded-2xl '>
            <div className=' '>
                <div className='w-full flex flex-col justify-items-center p-3'>
                    {!showCropper && !croppedImage && (<img className='h-40 self-center rounded-sm w-30' src={`http://localhost:3000/pics/${student.motherPic}`} />)}
...
export default CropMotherImage;
